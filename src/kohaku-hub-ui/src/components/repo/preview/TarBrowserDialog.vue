<!--
  ...
  Read-only browser for hfutils.index TAR + JSON pairs. Loads the
  sidecar index in full, builds an in-memory directory tree, and lets
  the user navigate paginated list / grid views inside the archive.
  Member preview routes to the same renderer the standalone blob page
  uses (image/video/audio/pdf/text/markdown), and member-level
  download triggers a browser-native save with the in-tar basename.

  No backend code path: the index JSON and member ranges are both
  fetched via /resolve/, the same way safetensors / parquet preview
  works (see FilePreviewDialog.vue). This keeps the indexed-tar
  surface consistent with the existing pure-client preview model.
-->
...
const props = defineProps({
  ...
  // top-of-modal warning / notice banner.
