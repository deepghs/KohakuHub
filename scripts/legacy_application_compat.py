#!/usr/bin/env python3
"""Compatibility repair for the immutable pre-017 application schema.
...
def repair_legacy_application_schema() -> bool:
    """Repair a database before the immutable 017 application boundary.

    The numbered 001-016 scripts are historical inputs and remain untouched.
    This helper is called by the current runner only when the database still
    has the pre-008 relation schema (or a known incomplete 008 result).
    """
